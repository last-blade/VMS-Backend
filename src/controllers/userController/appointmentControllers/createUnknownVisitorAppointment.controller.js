...
const createUnknownVisitorAppointment = asyncHandler(
  async (request, response) => {
    ...
    if (
      [plant, department, personToVisit].some(
        (input) => !isObjectIdValid(input),
      )
    ) {
    ...
    if (
      [appointmentDate, appointmentValidTill, purposeOfVisit].some(
    ...
    visitors.forEach((v, index) => {
      if (!v.mobile || !v.fullname) {
        throw new apiError(
          400,
          `Visitor ${index + 1}: mobile and fullname are required`,
        );
      }
    });

    const foundPlant = await Plant.findById(plant).populate(
      "company",
      "companyName",
    );
    ...
    const foundNewlyAppointent = await Appointment.findById(
